...
function build() {
  ...
  svg.Element.PathElementBase = function (node) {
    ...
    this.renderChildren = function (ctx) {
      ...
      if (ctx.fillStyle != '') {
        if (this.attribute('fill-rule').valueOrDefault('inherit') != 'inherit') {
  ...
  svg.Element.path = function (node) {
    ...
    this.PathParser = new (function (d) {
      ...
      this.getReflectedControlPoint = function () {
        ...
        // reflect point
        return new svg.Point(2 * this.current.x - this.control.x, 2 * this.current.y - this.control.y);
      };
              ...
              for (var s = 0; s < srcs.length; s++) {
                if (srcs[s].indexOf('format("svg")') > 0) {
                  ...
                  for (var f = 0; f < fonts.length; f++) {
                    svg.Definitions[fontFamily] = svg.CreateElement(fonts[f]);
                  }
                }
              }
            }
  ...
  svg.Element.feGaussianBlur = function (node) {
    ...
    this.apply = function (ctx, x, y, width, height) {
      ...
      stackBlur(ctx.canvas.id, x, y, width, height, this.blurRadius);
